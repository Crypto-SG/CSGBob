...
#	f.close()
	...
	# Scheduled jobs
	job_rollingBanLimit = jqueue.run_repeating(functions.rollingBanLimitLogic, interval=1800, first=0)
	job_getAdmins = jqueue.run_repeating(functions.getAdmins, interval=86400, first=0)
	job_rollingBanRemoval = jqueue.run_repeating(functions.rollingBanRemovalLogic, interval=10800, first=0)
	...
	setVoteLimit_handler = CommandHandler('setVoteLimit', functions.setVoteLimit, pass_args = True)
	getVoteLimit_handler = CommandHandler('getVoteLimit', functions.getVoteLimit)
	setBanLimit_handler = CommandHandler('setBanLimit', functions.setBanLimit, pass_args = True)
	getBanLimit_handler = CommandHandler('getBanLimit', functions.getBanLimit)
	getTimeOfBans_handler = CommandHandler('getTimeOfBans', functions.getTimeOfBans)
	getPending_handler = CommandHandler('getPending', functions.getPending)
	resetBanLimit_handler = CommandHandler('resetBanLimit', functions.resetBanLimit)
	...
	dispatcher.add_handler(resetBanLimit_handler)
